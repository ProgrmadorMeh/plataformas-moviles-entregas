...
function promedioEdades(listaDePersonas) {
    return listaDePersonas.reduce((total, persona) => total += persona["edad"],0) / listaDePersonas.length;
}
...
function cumplirAños(listaDePersonas) {
    listaDePersonas.forEach(persona => persona["edad"] += 1);
...
function laPersonaMayor(listaDePersonas) {
    ...
    for(let i=1; i<listaDePersonas.length;i++){
        if(mayor["edad"] < listaDePersonas[0]["edad"]) mayor = listaDePersonas[i];
    }
    return mayor;
}
...
function agregarHeladoFavorito(listaDePersonas, listaDeHelados) {
    ...
    return listaDePersonas.map(persona => {
      const helado = i < listaDeHelados.length ? listaDeHelados[i] : "vainilla";
      i++;
      return {
        ...persona,
